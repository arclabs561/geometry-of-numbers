...
lemma descent_lattice_covolume (n : ℕ) (u v : ℤ) (hn : 0 < n) :
  ...
  · -- Volume computation: `volume F = ofReal |det (Matrix.of B)| = n^2`.
    have hvol :
      ...
      simpa [F, B] using (ZSpan.volume_fundamentalDomain B)
...
lemma exists_sq_add_sq_add_one_eq_zero_mod_odd' (n : ℕ) (hn : Odd n) :
  ...
  simpa using Covolume.exists_sq_add_sq_add_one_eq_zero_mod_odd n hn
...
`Covolume/Legendre/Ankeny.lean`. We provide this lemma as an alias to avoid a duplicate `sorry`.
-/
theorem minkowski_sum_three_squares_of_three_mod_eight (n : ℕ) (hn : n % 8 = 3) :
    ∃ x y z : ℕ, x ^ 2 + y ^ 2 + z ^ 2 = n :=
  Covolume.sum_three_squares_of_three_mod_eight n hn
...
theorem minkowski_three_squares (n : ℕ) (_h : ¬ is_three_square_exception n) :
  ...
  simpa using Covolume.sum_three_squares_of_not_exception n _h
