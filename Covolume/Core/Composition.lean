...
three binary quadratic forms whose composition is identity in the class group.
...
- `slice_quadratic_form`: The binary quadratic form obtained by taking the determinant
  of a linear combination of slices.
-/
...
/-- Extract a 2x2 face of the cube. -/
...
/-- The binary quadratic form arising from a specific dimension of the cube.
    Q_i(x, y) = -det(M_0 * x - M_1 * y) where M_0, M_1 are the faces in dim i. -/
